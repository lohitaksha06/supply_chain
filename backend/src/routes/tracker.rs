...
/// GET /batch/:batch_id
/// Returns full details of a specific batch
async fn get_batch_details(
    State(pool): State<Arc<SqlitePool>>,
    Path(batch_id): Path<String>,
) -> Result<Json<BatchDetails>, (StatusCode, String)> {
    let row = sqlx::query(
        "SELECT id, batch_id, medicine_name, source, destination, timestamp, hash, previous_hash, signature, public_key
         FROM medicine_batches WHERE batch_id = ?"
    )
    .bind(&batch_id)
    .fetch_optional(pool.as_ref())
    ...
    if let Some(batch) = row {
        let timestamp: String = batch.try_get("timestamp").unwrap_or_default();
        Ok(Json(BatchDetails {
            id: batch.try_get("id").unwrap_or(0),
            batch_id: batch.try_get("batch_id").unwrap_or_default(),
            medicine_name: batch.try_get("medicine_name").unwrap_or_default(),
            source: batch.try_get("source").unwrap_or_default(),
            destination: batch.try_get("destination").unwrap_or_default(),
            timestamp: timestamp.clone(),
            expiry_date: timestamp,
            hash: batch.try_get("hash").unwrap_or_default(),
            previous_hash: batch.try_get("previous_hash").unwrap_or_default(),
            signature: batch.try_get("signature").ok(),
            public_key: batch.try_get("public_key").ok(),
...
pub fn tracker_routes(pool: Arc<SqlitePool>) -> Router {
    Router::new()
        ...
        // Batch details route
        .route("/batch/:batch_id", get(get_batch_details))
